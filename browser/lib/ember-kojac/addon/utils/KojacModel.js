 ...
 **/
export default KojacObject.extend({
	/**
	...
	setup: function(prop) {
		...
		for (var p in prop) {
			...
			if (_.isArray(propValue) && (propValue.length===2) && (KojacTypes.FieldTypes.indexOf(propValue[0])>=0)) {  // in form property: [Type, Default Value]
				this.__attributes[p] = propValue[0];
				prop[p] = propValue[1];
			} else if (KojacTypes.FieldTypes.indexOf(propValue) >= 0) {   // field type
				prop[p] = null;
				...
				var i = KojacTypes.FieldTypes.indexOf(KojacTypes.getPropertyValueType(propValue));
				if (i >= 0) {
					this.__attributes[p] = KojacTypes.FieldTypes[i];
				} else {
	...
	isAttribute: function(aName) {
		return this.constructor && this.constructor.__attributes && (aName in this.constructor.__attributes);
	},
	...
	attr: function(aName,aValue) {
		if (aName===undefined) {  // read all attributes
			...
			if (t)
				aValue = KojacTypes.interpretValueAsType(aValue,t);
